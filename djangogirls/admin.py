...
class PostModelAdmin(admin.ModelAdmin):
    ...
    def post_image(self, obj):
        if obj.feature_img:
            ...
            return ''
    fields = ['author', 'image',  'feature_img', 'title',
              'slug', 'text', 'category', 'tags', 'created_date',]
    prepopulated_fields = {"slug": ("title",)}
    list_filter = ['author', 'title', 'category', 'tags', 'published_date']
    list_display = ['author',  'post_image',
                    'post_title', 'category', 'published_date']
...
class UserModelAdmin(ExportActionMixin, admin.ModelAdmin):
    def profile(self, obj):
        if obj.user_profile:
            ...
            return ''
    list_display = ['first_name', 'username', 'profile', 'email',
                    'gender', 'dob', 'phone_no', 'city', 'state', 'zip_code', 'country']
    list_filter = ['first_name',  'email', 'gender', 'dob',
                   'phone_no', 'city', 'state', 'zip_code', 'country']
    search_fields = ['first_name',  'email', 'gender', 'dob',
                     'phone_no', 'city', 'state', 'zip_code', 'country']
